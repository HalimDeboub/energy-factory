...
try:
    from app.workflows.simple_workflow import SimpleEnergyWorkflow
    workflow = SimpleEnergyWorkflow()
    logger.info("✓ Using SimpleEnergyWorkflow")
except ImportError as e:
    logger.error(f"⚠️ Could not import workflow: {e}")
    workflow = None
...
# async def analyze_energy(query: Query):
    ...
    #     params = {"limit": 20, "order_by": "date desc"}
    ...
    #     logger.info(f"Received data: {latest}")
...
async def analyze_energy(query: Query):
    try:
        # Use the workflow instead of hardcoded logic
